        ...
        *::-webkit-scrollbar-thumb:hover {{
        ...
        @media (min-width: 1200px) {{
          .st-key-cgi_chat_dock {{
              position: fixed;
              bottom: 18px;
              left: calc(var(--cgi-leftpanel-width) + var(--cgi-gutter) + ( (100vw - var(--cgi-leftpanel-width) - var(--cgi-gutter)) / 2 ));
              transform: translateX(-50%);
              right: auto !important;
              width: min(1020px, calc(100vw - var(--cgi-leftpanel-width) - var(--cgi-gutter) - 4rem)) !important;
        ...
        .st-key-cgi_chat_dock div[data-testid="stTextInput"] input {{
        ...
        .st-key-cgi_chat_dock div[data-testid="stTextInput"] label {{
    ...
    if st.session_state.get(f"fb_open_{mid}", False):
        with st.container(key=f"fb_inline_form_{safe_mid}"):
            ...
            with st.form(key=f"fb_form_{mid}", clear_on_submit=True):
                ...
                corrected_sql = st.text_area("Correct SQL (optional)", key=f"fb_sql_{mid}") if mode == "sql" else None
        ...
        with st.container(key="cgi_main_panel"):
            st.markdown(
                ...
                <div class="cgi-center">
                  ...
                  <div class="muted">Ask questions about policies (RAG) and data (SQL).</div>
                </div>
                """,
                unsafe_allow_html=True,
            )
...
st.markdown("<div class='cgi-footer'>© 2026 CGI Inc.</div>", unsafe_allow_html=True)
